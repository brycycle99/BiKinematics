...
class Bike():
    ...
    def find_chainline(self):
        """
        Populates the list of chainline cogs with tuples: (point_name, pitch_circ_dia).
        Maps from Chainring -> Idler (if exists) -> Cassette.
        """
        # 1. Gather tooth counts from your specific JSON params
        try:
            teeth_ring = int(self.get_param('chainring_teeth'))
            teeth_cassette = int(self.get_param('cassette_teeth'))
        except (ValueError, TypeError):
            print("Error: Chainring or Cassette teeth params missing or invalid.")
            return

        # 2. Check for an idler in the points dictionary
        # We'll assume if an idler exists, you'll want a param for its teeth; 
        # otherwise, we default to a common size like 14.
        idler_point = next((p for p in self.points.values() if p.type == 'idler'), None)
        
        if idler_point:
            teeth_idler = int(self.get_param('idler_teeth')) if self.get_param('idler_teeth') else 14
            teeth_list = [teeth_ring, teeth_idler, teeth_cassette]
        else:
            teeth_list = [teeth_ring, teeth_cassette]

        # 3. Calculate Pitch Circle Diameter (PCD)
        # Using the formula: PCD = Pitch / sin(pi / N)
        link_len = 12.7
        pcd_values = link_len / np.sin(np.pi / np.array(teeth_list))

        # 4. Map PCDs to the actual geometry points
        # We clear the existing chainline to avoid duplicates on re-run
        self.chainline = []

        # Find the BB/Chainring point
        bb_point = next((p for p in self.points.values() if p.type == 'bottom_bracket'), None)
        if bb_point:
            self.chainline.append((bb_point.name, pcd_values[0]))

        # Find the Idler (if applicable)
        if idler_point:
            # The idler is the middle index in our pcd_values array
            self.chainline.append((idler_point.name, pcd_values[1]))

        # Find the Rear Wheel/Cassette point
        rw_point = next((p for p in self.points.values() if p.type == 'rear_wheel'), None)
        if rw_point:
            # The cassette is always the last index in our pcd_values array
            self.chainline.append((rw_point.name, pcd_values[-1]))
    ...
    def get_suspension_motion(self, travel, name):
        try:
            ...
            # THIS IS THE KEY: We print 'e' to see the actual Python error
            print(f"\n--- CRITICAL SOLVER ERROR ---")
            print(f"Error Type: {type(e).__name__}")
            print(f"Error Message: {e}")
            
            # If the error is inside SciPy, this will help us find the line
            import traceback
            traceback.print_exc() 
